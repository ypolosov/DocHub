...
Для хранения контрактов используется GitLab. Данная концепция реализует подход  
ArchOps (архитектура как код). Управление изменениями контрактов подчиняется gitflow.

Поддерживается OAuth авторизация через GitLab, что позволяет управлять доступом
пользователей к документации.
...
Для развертывание потребуется стандартная сборка VueJS приложения средствами npm.
```
npm install
npm run build
```
...
* В папке /serts разместите ваши сертификаты;
...
расположение подключаемых манифестов. При загрузки дерева документов, сначала загружается 
...
* Если не один из документов не выбран, то в области просмотра находится сводка, в которой представлены 
