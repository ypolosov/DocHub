...
...
```
Такое описание схемы позволяет указать любое строковое значение в них при описании объектов сущности. 
...
В этом случае в поле "type" необходимо указать, на какие объекты сущностей требуется сослаться. 
Делается это с использованием специальных типов DocHub имеющих структуру:

```
<идентификатор сущности>[.<тип поставляемого объекта>]
```

Допустим, необходимо, чтобы поля "from" и "to" заполнялись идентификаторами компонентов.
Тогда определение типа должно быть следующим:
...
...
  properties:
    from: 
      title: От кого
      type: components.component  # Указываем, что данное поле должно содержать идентификатор объекта сущности components
    to:     
      title: Кому
...
Для того, чтобы на объекты сущности могли ссылаться объекты других сущностей, необходимо их декларировать.
